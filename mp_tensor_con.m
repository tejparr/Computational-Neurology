...
Ap      = permute(A, pA);       % Perform permutation to reorder dimensions
...
A       = reshape(Ap, sA);      % Reshape the permuted A into the new array dimensions

% Expand B into full dimension space
%--------------------------------------------------------------------------
[~, pB] = sort(bi);             % Find indices to permute B so its axes are ordered by bi
Bp      = permute(B, pB);       % Perform permutation to reorder dimensions
...
B       = reshape(Bp, sB);      % Reshape the permuted B into the new array dimensions
